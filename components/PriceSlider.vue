<template>
    <div class="filter-section mb-5">
      <h3 class="filter-title text-base font-medium text-gray-900 mb-3">
        {{ isRent ? 'Price per month' : 'Property price' }}
      </h3>
      ...
      <div class="flex items-center space-x-2">
        <div class="flex-1 flex items-center border border-gray-300 rounded-md">
          ...
          <input 
            ...
            class="w-full p-2 border-none focus:ring-0"
            min="0"
          >
        </div>
  ...
  const sliderMax = computed(() => props.isRent ? 10000 : 5000000);
  ...
  const handleDrag = (e) => {
    ...
    if (props.isRent) {
      // Para alquiler, redondear a múltiplos de 50
      priceValue = Math.round((percentPosition * priceRange + sliderMin.value) / 50) * 50;
    } else {
      // Para venta, redondear a múltiplos de 5000
      priceValue = Math.round((percentPosition * priceRange + sliderMin.value) / 5000) * 5000;
    }
    ...
    if (draggingHandle.value === 'min') {
      ...
      const step = props.isRent ? 50 : 5000;
